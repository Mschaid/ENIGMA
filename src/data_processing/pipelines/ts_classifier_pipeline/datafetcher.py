...
import re
from typing import Any, Dict, List, Literal, NewType, Protocol, Callable
...
def pad_dict_arrays(data_dict: Dict[str, np.array], pad_val=-100) -> Dict[str, np.array]:
    """ formats the dictionary of data to have the same length"""
    max_length = get_max_value_length(data_dict)
    padded_dict = {k: np.pad(
        v, (0, max_length - v.shape[0]), constant_values=-100) for k, v in data_dict.items()}
    return padded_dict
...
# fetch files to read
    ...
    def load_categorical_data_into_ts_dataframe(self):
        ...
        updated_df = (
            pl.concat([self.timeseries_dataframe, cat_df], how='horizontal')
    ...
    def write_to_parquet(self):
        dir_to_save = self.path_to_metadata.parents[1] / \
            "compiled_timeseries_data"
