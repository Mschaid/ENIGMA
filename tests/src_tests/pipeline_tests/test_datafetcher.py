...
from src.data_processing.pipelines import (GuppyDataFetcher,
                                           filter_metadata_keys_for_keywords,
...
def test_filter_metadata_paths_for_keywords():
    ...
    filtered_values = filter_metadata_keys_for_keywords(
        'filter_key', metadata=test_metadata)
    assert filtered_values == ['value5', 'value6', 'value7', 'value8']


def test_read_hdf5_event_timestamps_data():
    test_path = Path("tests/data/test_data.hdf5")
    timestamps = read_hdf5_event_timestamps_data(test_path)
